...
var (
	...
	On OnOff = conversion.Bool(true)
	// Off indicates a setting is disabled in NGINX
	Off OnOff = conversion.Bool(false)
)
...
type Global struct {
	...
	// +optional
	ProxyRealIPCIDR []*IPAddr `json:"proxy-real-ip-cidr,omitempty"`
	...
	// +optional
	StatusIPV4Whitelist []*IPAddr `json:"status-ipv4-whitelist,omitempty"`
	// +optional
	StatusIPV6Whitelist []*IPAddr `json:"status-ipv6-whitelist,omitempty"`
	...
	// +optional
	WhitelistSourceRange []*IPAddr `json:"whitelist-source-range,omitempty"`
}
...
func (ipa *IPAddr) DeepCopyInto(out *IPAddr) {
	out.IP = ipa.IP
	out.Zone = ipa.Zone
}

func (ipa *IPAddr) DeepCopy() *IPAddr {
	return &IPAddr{IPAddr: net.IPAddr{IP: ipa.IP, Zone: ipa.Zone}}
}
